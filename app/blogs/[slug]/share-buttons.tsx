...
export function ShareButtons({ title }: { title: string }) {
  ...
  const btnClass =
    "flex items-center justify-center gap-2 rounded-xl border border-white/10 py-2.5 px-3 text-[10px] font-bold uppercase tracking-widest text-white/50 transition-colors hover:border-[#B08CFF] hover:text-[#B08CFF]";
  ...
  return (
    <div className="rounded-2xl bg-[#0A1628] p-6 outline outline-1 -outline-offset-1 outline-white/10">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <button onClick={handleLinkedin} className={`${btnClass} col-span-2`}>
          <svg
            className="h-3.5 w-3.5 shrink-0"
            viewBox="0 0 24 24"
            fill="currentColor"
